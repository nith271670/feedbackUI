...
export class DashboardComponent implements OnInit {
 ...
 locale={
  ...
  ngModelChange(date){
    ...
    ////console.log(moment(date.startDate._d).format('YYYY-MM-DD'))
...
for(let trng in this.TrainingList){
  //console.log(this.TrainingList[trng].trainers);
if(this.TrainingList[trng].training == training){
  console.log(this.TrainingList[trng].trainers)

  this.trainerList = this.TrainingList[trng].trainers;
}
...
//console.group("for loop");
    for(let i=0;i<this.feedBackListGrouped[j].trainingData.length;i++){
      //console.group("for loop "+i);
//console.log(this.feedBackListGrouped[j].trainingData[i].questions[0].subquestions[0].rating)
...
//console.log(this.trainingContent[j]);
//console.log(this.trainingExperience[j]);
